// Source : https://github.com/imagicbell/piano-app/blob/a22138d05361e1ebf2571eed2949b0e4544c2781/src/data/midi/musicxmlParser.js
...
import {allNotes as noteMap} from '../keyboard/notes';
...
const parseHeaderInfo = (data: any, header: Header, measures: Measure[]): void => {
    let curTempo: Tempo | undefined;
    let curTimeSignature: TimeSignature | undefined;
    let repeatMeta: RepeatMeta | undefined;
    ...
    while (measureIndex < data.measures.length) {
        ...
        if (measureIndex > 0) {
            ...
            let elapsedSeconds = (60 / (curTempo.bpm ?? 60)) * curTimeSignature.beats; // TODO curTempo.bpm peut être undefined
            ...
            if (info._class === "Attributes") {
                ...
                if (velocity) {
                    if (!header.velocities) {
                        header.velocities = [] // ajout Bludwarf
                    }
        ...
        if (curTempo && curTempo.tempo && curTimeSignature && curTempo.bpm === undefined) {
            curTempo.bpm = (curTimeSignature.beatType / 4) * curTempo.tempo;
        }

        doRepeat(data.measures, repeatMeta as any); // TODO typage repeatMeta
        measureIndex++;
    }
}

const parseSound = (soundInfo: any, measures: number, time: number): [Tempo | undefined, Velocity | undefined] => {
    let tempo: Tempo | undefined;
    let velocity: Velocity | undefined;
    if (soundInfo.tempo) {
        tempo = {
...
const parseRepeatMeta = (repeatInfo: any, measures: number, repeatMetaP: RepeatMeta | undefined): RepeatMeta => {
    const repeatMeta: RepeatMeta = repeatMetaP || {} as any; // TODO ajout bludwarf : any
...
export const parseTracks = (data: any, header: Header, measures: Measure[]): [Track[], number] => {
    ...
    data.partList.forEach((part: any, partIndex: number) => { // TODO typage part
        tracks[partIndex] = {
    ...
    let totalDuration = lastMeasureTime + (60 / (lastBpm ?? 60)) * lastBeats; // TODO lastBpm peut être undefined
...
const parseNotes = (data: any, header: Header, measures: Measure[], tracks: Track[]): void => {
    ...
    data.measures.forEach((measure: any, measureIndex: number) => { // TODO typage measure
        curTempo = (header.tempos as any).findLast((t: any) => t.measures <= measureIndex); // TODO typage measure et t
        curVelocity = (header.velocities as any).findLast((v: any) => v.measures <= measureIndex); // TODO typage velocities et v

        data.partList.forEach((part: any, partIndex: number) => {// TODO typage measure
            let notes = tracks[partIndex].notes;
            let curTime = measures[measureIndex].time;

            measure.parts[part.id].forEach((info: any) => {// TODO typage measure
                switch (info._class) {
                    case "Attributes": {
                        //get time for one division
                        if (partIndex === 0 && info.divisions) {
                            curDivisionTime = 60 / (curTempo.tempo ?? 60) / info.divisions;  // TODO curTempo.tempo peut être undefined
                        }
                        break;
                    }
                    case "Backup": {
...
const parseNote = (noteInfo: any, notes: Note[], tieNotes: Note[], divisionTime: number, time: number, velocity: number): void => {
    ...
    if (noteInfo.ties) {
        if (noteInfo.ties[0].type === 0) {
            //start
            note = {} as any; // TODO ajout bludwarf : as any
            tieNotes.push(note);
        } else {
            //stop
            note = tieNotes.shift() as any; // TODO ajout bludwarf : as any
    ...
    if (noteInfo.pitch.alter) {
        ...
        note.name = noteName;
        if (noteMap) {
            const foundNote = noteMap.find(n => n.ansi === noteName);
            if (foundNote) { // TODO ajout Bludwarf
                note.midi = foundNote.midi;
            }
        }
    }
