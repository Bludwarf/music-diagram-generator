...
export class Structure {
    ...
    get patternsWithoutDuplicatedInitial(): Pattern[] {
        const initials: string[] = [];
        const allPatterns = this.partsInStructure.flatMap(partInStructure =>
            partInStructure.part.sections.flatMap(section => section.patterns)
        );

        const patternsWithoutDuplicatedInitial: Pattern[] = [];
        for (const pattern of allPatterns) {
            if (!initials.includes(pattern.initial)) {
                patternsWithoutDuplicatedInitial.push(pattern);
                initials.push(pattern.initial);
            }
        }
        return patternsWithoutDuplicatedInitial;
    }
